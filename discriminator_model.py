...
class DiscriminatorModel:
    def __init__(self, args, input_image, truth_image, generated_image):
        self.truth_output = self.__create_prediction_model(args, input_image, truth_image, False)
        tf.get_variable_scope().reuse_variables()
        self.generated_output = self.__create_prediction_model(args, input_image, generated_image, True)
    ...
    @staticmethod
    def __create_prediction_model(args, input_image, output_image, reuse):
        with tf.variable_scope("input", reuse=reuse):
            input_fully_connected_activation = \
                DiscriminatorModel.__create_image_to_fully_connected_layers(args, input_image, reuse)

        with tf.variable_scope("output", reuse=reuse):
            output_fully_connected_activation = \
                DiscriminatorModel.__create_image_to_fully_connected_layers(args, output_image, reuse)

        all_fully_connected_activations = \
            tf.concat([input_fully_connected_activation, output_fully_connected_activation], axis=1)

        weights_to_output = tf.get_variable(
            shape=[args.fully_connected_size * 2, 1],
        ...
        return tf.nn.sigmoid(tf.matmul(all_fully_connected_activations, weights_to_output) + biases_to_output)

    @staticmethod
    def __create_image_to_fully_connected_layers(args, image, reuse):
        current_input = image
